...
exports.usersController = {
    ...
    getUserSeeds: async (req, res) => {
        try {
            ...
            const seedIds = await contractService_1.contractService.getUserSeeds(address);
            // Get full seed data for each seed
            const seeds = [];
            for (const seedId of seedIds) {
                const seedData = await contractService_1.contractService.getSeedData(seedId);
                if (seedData) {
                    const seed = await seedTransformService_1.seedTransformService.transformContractDataToSeed(seedData);
                    seeds.push(seed);
                }
            }
            res.json({
     ...
     */
    getUserSeedsCount: async (req, res) => {
        try {
            const { address } = req.params;
            if (!address || !address.startsWith('0x')) {
                res.status(400).json({
            ...
            const count = await contractService_1.contractService.getUserSeedsCount(address);
            res.json({
                success: true,
                count,
                owner: address,
     ...
     */
    getUserSnapshots: async (req, res) => {
        try {
            const { address } = req.params;
            if (!address || !address.startsWith('0x')) {
                res.status(400).json({
            ...
            const snapshotIds = await contractService_1.contractService.getUserSnapshotIds(address);
            const snapshots = [];
     ...
     */
    getUserSnapshotsCount: async (req, res) => {
        try {
            const { address } = req.params;
            if (!address || !address.startsWith('0x')) {
                res.status(400).json({
     ...
     */
    getUserSnapshotData: async (req, res) => {
        try {
            const { address } = req.params;
            if (!address || !address.startsWith('0x')) {
                res.status(400).json({
            ...
            const snapshotData = await contractService_1.contractService.getUserSnapshotData(address);
            res.json({
                success: true,
    ...
    getUserBalance: async (req, res) => {
        try {
            ...
            const balance = await contractService_1.contractService.getUserPoolBalance(address);
            res.json({
                success: true,
    ...
    getUserStats: async (req, res) => {
        try {
            ...
            const seedIds = await contractService_1.contractService.getUserSeeds(address);
            const snapshotIds = await contractService_1.contractService.getUserSnapshotIds(address);
            const poolBalance = await contractService_1.contractService.getUserPoolBalance(address);
            const seedNFTBalance = await contractService_1.contractService.getUserSeedNFTBalance(address);
            const snapshotNFTBalance = await contractService_1.contractService.getUserSnapshotNFTBalance(address);
            res.json({
                success: true,
                stats: {
    ...
    getUserPortfolio: async (req, res) => {
        try {
            ...
            for (const snapshotId of snapshotIds) {
                const snapshotData = await contractService_1.contractService.getSnapshotData(snapshotId);
                if (snapshotData) {
                    snapshots.push({
                        id: snapshotId,
                        ...snapshotData,
